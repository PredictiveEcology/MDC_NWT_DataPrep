...
#    `functionName` is a function's name in your module (e.g., `MDC_NWT_DataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `MDC_NWT_DataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("MDC_NWT_DataPrep")
  path <- list(modulePath = "..",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    MDC_NWT_DataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("MDC_NWT_DataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- MDC_NWT_DataPrepEvent1(mySim)
  } else {
  ...
  if (exists("MDC_NWT_DataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- MDC_NWT_DataPrepEvent2(mySim)
  } else {
